                        ...
                        for i in matchedIdxs:
                            ...
                            if len(entries) > 0 :
                                try :
                                    ...
                                    if not any(d['name'] == name.strip() for d in entries): # check name if exist in any element
...
class Ui(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle('ABSENSI WAJAH MENGGUNAKAN RASPBERRY-PI (BY FAJARLABS)')
    ...
    def appendAgenda(self, name):
        agenda = ''
        if name == 'Fajar' :
            agenda = 'Jam 12:30 Ketemu client di gedung BCA untuk membahas mesin ATM'
        if name == 'ElonMusk':
            agenda = 'Jam 14:00 Membahas tentang pembayaran bitcoin bersama direktur tesla'
...
if __name__ == "__main__":
    # load data from database, but this example using static data
    user_list = [
